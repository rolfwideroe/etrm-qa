...
select
year(p.tradedate) as YearNumber,datepart(M,p.tradedate) as MonthNumber, datepart(D,p.tradedate) as DayNumber, avg(p.AvgPrice*c1.Rate*c2.Rate) as BaseValue ,min(p.tradedate)as StartOfMonth,max(p.tradedate) as EndOfMonth,COUNT(p.AvgPrice) as NumberOfDays
...
group by year(tradedate), datepart(M,tradedate), datepart(D,tradedate)
order by min(tradedate)


select
year(p.tradedate) as YearNumber,datepart(M,p.tradedate) as MonthNumber,datepart(D,p.tradedate) as DayNumber, avg(p.AvgPrice*c1.Rate*c2.Rate) as PeakValue ,min(p.tradedate)as StartOfMonth,max(p.tradedate) as EndOfMonth,COUNT(p.AvgPrice) as NumberOfDays
from ECMAreaPrices p
join CurrencyRates c1
on c1.RateDate=p.TradeDate
