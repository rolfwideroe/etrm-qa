...
namespace TestLiveCurveScheduling
{
    class TestWCFGetPriceCurveByCriteria
    {
        ...
        public static void ProcessQuery(string testFilepath)
        {
            ...
            {
                Parameters = new PriceBoardCustomPriceFeedParameters
                {
        ...
        public static void GetPriceCurveByCriteria(LiveCurveSchedulingTestCase testCase)
        {
            ...
            const double pointDeliveryTolerance = 1e-4;
            ...
            if (testCase.Commodity.Equals("Emission") || testCase.Commodity.Equals("El.certificate"))
                tolerance = pointDeliveryTolerance;
            ...
            for (int i = 0; i < testCase.LivePrices.Length; i++)
            {
                ...
                try
                {
                    ...
                    if (Math.Abs(actualPrice) >= tolerance || Math.Abs(l.Close) >= tolerance)
                    {
                        ...
                        //Assert value
                        Assert.AreEqual(0, relativeError, tolerance, errorMessage);
                    }
