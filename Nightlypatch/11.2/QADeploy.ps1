...
$Servers=@("BERVS-QA112A","QA112CR01","QA112CR02","QA112AR03","IS-TEST-MF")

$Clients=@("BERVS-QA112C","","","QA112CR03","CLIENTTESTXP")
$WSMPath="\\$DEployServer\INSTALLATIONS\DEP_TEMP\WSM\2012.1\WSM2012.1.msi"
$WSMLocalpath="C:\Elviz\WSM2012.1.msi"
...
WHILE($c -lt $Servers.Length){
    if($Servers[$c] -ne $null){
        IF($CompName -eq $Servers[$c]){
            $IS_NET_API_LOGON_USERNAME1="Bradyplc\$qainstall"
            $IS_NET_API_LOGON_PASSWORD1="$qainstallpwd"
            $SQL_SERVER1="BERSV-SQL8REG"
            $SQL_USER1="EcmDbUser"
            $SQL_PASSW1="EcmDbQaReg"

            $DB_SYSTEM1="QAREGSystem112"
            $DB_ECM1="QAREGECMOLD112"

            $DB_PRICES1="QAREGPRICES112"
            $DB_DWH1="QADataWarehouse112"
            ...
            $QAClient="\\$QAClientR"
            $exist=$true
        IF($Servers[$c] -eq "QA112AR02"){ 
                $IS_NET_API_LOGON_USERNAME1="Bradyplc\$qainstall"
                $IS_NET_API_LOGON_PASSWORD1="$qainstallpwd"
                $SQL_SERVER1="BERSV-SQL8REG"
                $SQL_USER1="EcmDbUser"
                $SQL_PASSW1="EcmDbQaReg"
                $DB_SYSTEM1="QASystem_Reg112"
                $DB_ECM1="QAECM_REG112"
                $DB_PRICES1="QAPrices_Reg112"
                $DB_DWH1="QADatawareHouse_Reg112"
                $DB_FWC1="VizForwardCurve112"
                ...
                $exist=$true
                Break
            }ELSEIF(($Servers[$c] -eq "BERVS-QA112A") -or ($Servers[$c] -eq "IS-TEST-MF")){ 
                $IS_NET_API_LOGON_USERNAME1="Bradyplc\$qainstall"
                $IS_NET_API_LOGON_PASSWORD1="$qainstallpwd"
                $SQL_SERVER1="BERSV-SQL8QA"
                $SQL_USER1="EcmDbUser"
                $SQL_PASSW1="EcmDbUser"
                $DB_SYSTEM1="QASystem112"
                $DB_ECM1="QAECM112"

                $DB_PRICES1="QAPRICES112"
                $DB_DWH1="VizDataWarehouse112"
                $DB_FWC1="VizForwardCurve112"
...
IF ($QAServerR -eq "QA112A"){
    ...
    IF(Test-Path "\\$DeployServer\INSTALLATIONS\QASCRIPTS\$Version\Config_Files\PriceboardServerConfig\MontelApiFeeder.xml"){
	 ...
	 IF(Test-Path "\\$DeployServer\INSTALLATIONS\QASCRIPTS\$Version\Config_Files\AppSettings.config" ){
        Echo_msg "$output_file" "[INFO]:Replacing  Bin\Configuration\ClientConfig\AppSettings.config"
        ...
        Echo_msg "$output_file" "[ERROR]:\\$DeployServer\INSTALLATIONS\QASCRIPTS\$Version\Config_Files\AppSettings.config IS NOT EXIST"
    }
}
...
Echo_msg "$output_file" "[INFO]-Reinstalling Web sales Manager"
	...
	&cmd /c "msiexec /i `"$WSMLocalpath`" /quiet INSTALLDIR=C:\Elviz APP_SERVER=$QAServerR IS_SQLSERVER_SERVER=BERSV-SQL8QA IS_SQLSERVER_USERNAME=EcmDbUser IS_SQLSERVER_PASSWORD=EcmDbUser IS_SQLSERVER_DATABASE=QAWSM112 PORT_NUMBER=8009 BUYATC=1 BUYEC=0 USEFAILEDCURVES=1 USENONSYNCEDCURVES=1" | Out-Null
...
-------------------------------------------------------------------
 ...
 #   }
    ...
    Echo_msg "$output_file" "[INFO]:DISABLE The QA_DEPLOY scheduled tasks to prevent it to run after a normal reboot" 
    & schtasks /Change /ru bradyplc\$qainstall /rp $qainstallpwd  /tn Run_QADEPLOY /DISABLE
...
-------------------------------------------------------------------
    " 
    Echo_msg "$output_file" "$output_file" $msg2
    Send_End_Message
}
...
IF (($QAServerR -eq "QA112A")){
	...
	$buildDefinition = "11.2_IntegrationTests"
	$teamProject = "Elviz"
	...
	$latestBuildDetail = $buildServer.QueryBuilds("Elviz", "11.2_Elviz_Release")[-1]	
