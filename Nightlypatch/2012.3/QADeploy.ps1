...
$setup="setup.exe" 
$Version="2012.3"
$VersionFolder="2012.3"
$qainstall="qainstall"
$qainstallpwd="installqa"
$DEployServer="BERSVDEPLOY"
$Servers=@("QA123A","QA123AR01","IS-TEST-MF")
$Clients=@("QA123C","QA123CR01","CLIENTTESTXP")
...
$WSMPath="\\$DEployServer\INSTALLATIONS\DEP_TEMP\WSM\WSM12.1\WSM.msi"
...
WHILE($c -lt $Servers.Length){
    if($Servers[$c] -ne $null){
        IF($CompName -eq $Servers[$c]){
            ...
            $DB_SYSTEM1="QAREGSystem123"
            $DB_ECM1="QAREGECMOLD123"
            $DB_PRICES1="QAREGPRICES123"
            $DB_DWH1="QADataWarehouse123"
            ...
            $QAClient="\\$QAClientR"
            $exist=$true
        IF($Servers[$c] -eq "QA123AR01"){ 
                $IS_NET_API_LOGON_USERNAME1="Viz\$qainstall"
                $IS_NET_API_LOGON_PASSWORD1="$qainstallpwd"
                $SQL_SERVER1="BERSV-SQL8REG"
                $SQL_USER1="EcmDbUser"
                $SQL_PASSW1="EcmDbQaReg"
                $DB_SYSTEM1="QASystem_Reg123"
                $DB_ECM1="QAECM_REG123"
                $DB_PRICES1="QAPrices_Reg123"
                $DB_DWH1="QADatawareHouse_Reg123"
                $DB_FWC1="VizForwardCurve123"
                #------- Machines names -----------
                $QAServerR=$Servers[$c]
                $QAClientR=$Clients[$c]
                # ***** to UNC path 
                ...
                $exist=$true
                Break
            }ELSEIF(($Servers[$c] -eq "QA123A")){ 
                $IS_NET_API_LOGON_USERNAME1="Viz\$qainstall"
                $IS_NET_API_LOGON_PASSWORD1="$qainstallpwd"
                $SQL_SERVER1="BERSV-SQL8QA"
                $SQL_USER1="EcmDbUser"
                $SQL_PASSW1="EcmDbUser"
                $DB_SYSTEM1="QASystem123"
                $DB_ECM1="QAECM123"
                $DB_PRICES1="QAPRICES123"
                $DB_DWH1="VizDataWarehouse123"
                $DB_FWC1="VizForwardCurve123"
                #------- Machines names -----------
                $QAServerR=$Servers[$c]
                $QAClientR=$Clients[$c]
...
IF ($QAServerR -eq "QA123A"){
    ...
    IF(Test-Path "\\$DeployServer\INSTALLATIONS\QASCRIPTS\$Version\Config_Files\PriceboardServerConfig\MontelApiFeeder.xml"){
	 ...
	 IF(Test-Path "\\$DeployServer\INSTALLATIONS\QASCRIPTS\$Version\Config_Files\AppSettings.config" ){
        ...
        Copy-Item  "\\$DeployServer\INSTALLATIONS\QASCRIPTS\$Version\Config_Files\AppSettings.config" "C:\Elviz\Bin\Configuration\ServerConfig\" -Force 
    }ELSE{
...
IF ($QAServerR -eq "QA123A"){
	...
	&cmd /c "msiexec /i `"c:\Elviz\WSM.msi`" /quiet INSTALLDIR=C:\Elviz APP_SERVER=$QAServerR IS_SQLSERVER_SERVER=BERSV-SQL8QA IS_SQLSERVER_USERNAME=EcmDbUser IS_SQLSERVER_PASSWORD=EcmDbUser IS_SQLSERVER_DATABASE=QAWSM123 PORT_NUMBER=8009 BUYATC=1 USEFAILEDCURVES=1 USENONSYNCEDCURVES=1" | Out-Null
...
IF (($QAServerR -eq "QA123A")){
	...
	$buildDefinition = "2012.3_IntegrationTests"
	$teamProject = "Elviz"
