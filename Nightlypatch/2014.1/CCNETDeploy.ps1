...
$msiFilesource="\\BERSVDEPLOY\INSTALLATIONS\DEP_TEMP\CCNET\2014.1\CCNET2014.1.msi"
$msiFileapth= "C:\CCNET2014.1.msi"
...
$DB_CC="QACCNET141"
$MSGQPORT="61616"
$PRIMARYIP="194.110.101.26"
$SOCKETCONNECTPORT="37438"
$SOCKETUSER="PIXML_VIZ"
$SOCKETPASSW="Nasdaq13"
$SENDERSUBID="XVIZ"
$SENDERCOMPID="NCVIZ"
$TARGETCOMPID="GENIUM_TEST"
$ELVIZRESPQUEUE="dealinsert.response.net.ish121"
$now_date=Get-Date
...
Echo_msg "$LogFileSpecified" "--------------------------------------------------------------- $nl"
	...
	$sNames =@("ElvizContractClearer","ElvizTrayportadapter","ElvizNasdaqOMXadapter","ActiveMQ")
...
IF( $product -eq $null){
	...
	Echo_msg "$LogFileSpecified" "[INFO]-Stopping NasdaqOMX Service."
	& c:\util\pskill.exe "NordpoolAdapter.exe"
	...
	$nasdaqOmxService= "ElvizNasdaqOMXadapter"
	$command = "sc.exe config $nasdaqOmxService start= demand"
	$Output = Invoke-Expression -Command $Command -ErrorAction Stop
	if($LASTEXITCODE -ne 0){
	    Echo_msg "$LogFileSpecified" "[ERROR]Failed to set $nasdaqOmxService to Manual Start "
				$Error.Add("[ERROR]Failed to set $nasdaqOmxService to Manual start. ")
	} else {
	   Echo_msg "$LogFileSpecified" "[SUCESSS]$nasdaqOmxService is modified to run Manually "
	}

	$ServicesNames =@("ElvizTrayportadapter","ElvizContractClearer")
